    ...
    <ScreenWrapper className="pb-4">
      ...
      <View className="flex-row items-center justify-between my-3">
        <View className="flex-row items-center gap-3">
          ...
          <View>
            <Text className="text-white text-xl font-black">Retail Orders</Text>
            <Text className="text-zinc-400 text-xs">{orders.length} transactions recorded</Text>
          </View>
        </View>
      ...
      <Card className="bg-zinc-900 border-zinc-800 p-3.5 mb-3 flex-row items-center justify-between">
        <View>
          ...
          <Text className="text-emerald-400 font-black text-xl mt-1">
            {formatTaka(totalRevenue)}
          </Text>
          <Text className="text-zinc-500 text-[10px] mt-0.5">{orders.length} total orders</Text>
        </View>
        <View className="w-11 h-11 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 items-center justify-center">
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Card className="bg-zinc-900/60 border-zinc-800 p-8 items-center justify-center my-6">
              ...
              <Text className="text-zinc-300 font-bold text-sm">No orders found</Text>
            ...
            return (
              <Card className="bg-zinc-900 border-zinc-800 p-3.5 mb-2.5">
                ...
                {parsedItems.length > 0 && (
                  <View className="mt-2.5 pt-2 border-t border-zinc-800/80">
                    {parsedItems.map((prod, idx) => (
                      <View key={idx} className="flex-row justify-between py-0.5">
                        <Text className="text-zinc-300 text-xs flex-1 mr-2" numberOfLines={1}>
                ...
                {isConfirmed && (
                  <View className="mt-3 pt-2.5 border-t border-zinc-800 flex-row justify-end">
                    <Pressable
                      onPress={() => handleMarkDelivered(item.id)}
                      className="px-3 py-1.5 rounded-lg bg-emerald-600 active:bg-emerald-700"
                    >
